...
std::vector<std::vector<float> > sphereLogic2(std::vector<std::vector<std::vector<float> > > sphere_locs_3d){
	...
	std::vector<std::vector<float> > sphere_locs (0,std::vector<float>(6, 0));
	...
	float z_threshold = 0.03; //0.02
	float y_threshold = 0.03; //0.02
...
int main (int argc, char** argv)
{
    ...
    ss5 << "stitched_clouds3/plants_tform_" << argv[1] << ".ply";
    ...
    for(int n=0; n<num_slices; n++){
		...
		for(int i=0; i<hotspots.size(); i++){
			if(hotspots[i][0]!=0){
				//cout << "Color: " << hotspots[i][2]/200 << endl;
	...
	viewer->setBackgroundColor (.5, .5, 1);
