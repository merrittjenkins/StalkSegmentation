...
void pairAlign (const PointCloud<pcl::PointXYZRGB>::Ptr cloud_src, const PointCloud<pcl::PointXYZRGB>::Ptr cloud_tgt, PointCloud<pcl::PointXYZRGB>::Ptr output, Eigen::Matrix4f &final_transform, bool downsample = false)
{
  //
  ...
  int i=0;
  while (reg.getMaxCorrespondenceDistance() > 0.05)
  {
	i = i+1;
   ...
   //----------------------------------CONFIGURE STEREO AND CAMERA SETTINGS----------------------------------
	...
	char img3_filename[100];
 	...
 	for (int w=0; w<5; w++)
 	{
		...
		/*
		scaleImage scaledown; //(img2_filename, color_mode);
		//cout << buffer << endl;
		Mat img = scaledown.downSize(buffer, color_mode);
		//cout << buffer << endl;
	...
	//---------------------------------------------------OFFSET CLOUD AND WRITE TO FILE-------------------------------------------
	    ...
	    pcl::io::savePLYFile (ss.str (), *cloud_filtered, true);
	    cout << "writing..." << endl;
